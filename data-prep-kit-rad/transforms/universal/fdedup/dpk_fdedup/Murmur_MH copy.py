...
class Murmur_MH:
    ...
    def minhash2_nosalt1(self, shingles: List[str], doc_len: int, doc_id: int):
        """
        for each shingle (i.e. a group of k-words) it generates a digest value based on
        mmh3-hash function (32-bit)
    ...
    def minhash2_nosalt_new(self, shingles: List[str], doc_len: int, doc_id: int,preprocessed: bool):
        """
        for each shingle (i.e. a group of k-words) it generates a digest value based on
        mmh3-hash function (32-bit)
        ...
        if preprocessed:
            ...
            data=np.right_shift(
                    (hash_values * np.tile(self.permutations, (len(hash_values), 1)).T).T,
                    32,
                ).astype(np.uint32).min(axis=0).tolist()
            # for hash_value in data:
            #     inverted_index.insert_hash(hash_value, doc_id)  # Insert with count 1 for each occurrence

            return (data,doc_len,doc_id)
    ...
    def jaccard(mh1: np.array, mh2: np.array) -> float:
        ...
        assert len(mh1) == len(mh2)
        # return np.count_nonzero(mh1 == mh2) / len(mh1)
        return len(np.intersect1d(mh1,mh2)) / len(mh1)
