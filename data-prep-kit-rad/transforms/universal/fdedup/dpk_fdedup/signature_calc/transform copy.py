...
class SignatureCalculationTransform(AbstractTableTransform):
    ...
    def emit_bands1(self, int_id_column: str, minhashes: np.array, doc_length: int, b: int, r: int, seed: int = 42):
        num_minhashes = len(minhashes)
        # print("=======minhashes=======")
        # print(minhashes)
        assert b * r <= num_minhashes, f"b*r must be <= num minhashes, was b={b}, r={r}, num_minhashes={num_minhashes},int_id_column={int_id_column} "
        results = []
        # print("=======minhashes=======")
        # print(minhashes)
        for band_index in range(b):
            ...
            slice_bytes = minhashes[band_index * r : (band_index + 1) * r].tobytes()
            ...
            # )
            # print("=======XXXX=======")
            # print(minhashes[band_index * r : (band_index + 1) * r])
            # print(band_hash)
    ...
    def transform(self, table: pa.Table, file_name: str = None) -> tuple[list[pa.Table], dict[str, Any]]:
        ...
        #     lambda row: mm_min_hash.minhash2_nosalt_new(
        ...
        # print(permutations)
        # print(permutations.tolist())
        # print(np.array(permutations.tolist()))
        ...
        print("df.columnsdf.columnsdf.columnsdf.columns")
        print(df.columns)

        # generate minhash values
        if 'minhashes' in df.columns and 'document_length' in df.columns:

            minhashes = df.map_rows(
                lambda row: mm_min_hash.minhash2_nosalt_new(
                    *self._generate_word_shingles(row, inverteFileIndex,window_size=self.word_shingle_size)
                )
            )
        ...
        print(f"Elapsed time>>>>>>>>>>>>>>>>>>>STEP 1MinHash & Shingling>>>>>>>>>>>>>>>>>>>>>>>>>>>>>: {elapsed_ns} ns")
        ...
        # print(self.all_band_hashes.columns)
        # print(self.all_band_hashes)
        # print(self.all_minhashes.columns)
        # print(self.all_minhashes)
        ...
        # Compute and print elapsed nanoseconds
        elapsed_ns = t_end - t_start
        print(f"Elapsed time>>>>>>>>>>>>>>>>>>>STEP 2 Banding>>>>>>>>>>>>>>>>>>>>>>>>>>>>>: {elapsed_ns} ns")
        ...
        # data = pl.DataFrame({
        #     "key": inverteFileIndex.index.keys(),
        #     "value": [  # Convert inner dicts into serializable format
        #         {"inner_key": list(v.keys()), "inner_value": list(v.values())} for v in inverteFileIndex.index.values()
        #     ]
        # })
        # self.all_minhashes.write_parquet("/Users/nelson/workspace/Research/DataPreprocessing/ibm/active/data-prep-kit/transforms/universal/fdedup/python/src/all_minhashes.parquet")
        # self.all_band_hashes.write_parquet("/Users/nelson/workspace/Research/DataPreprocessing/ibm/active/data-prep-kit/transforms/universal/fdedup/python/src/all_band_hashes.parquet")
        # inverteFileIndex.save_index()
    ...
    def write_band_signatures(self):
        ...
        segment_bounds_list.append(upper_bound)
        segment_bounds = np.array(segment_bounds_list, dtype=np.uint64)
        self.logger.info(f"Calculated {len(segment_bounds)} segment_bounds")
        # output stats for the metadata
        num_tables_written = 0
        num_docs_written = 0
        num_bytes_written = 0
        self.logger.info(f"dataframe self.all_band_hashes has {len(self.all_band_hashes)} rows")
        self.logger.info(f"dataframe self.all_minhashes has {len(self.all_minhashes)} rows")
        # iterate through the bands, get the band hashes for each band, divide
        # them into segments, join with minhashes, and upload to storage
        for band_ix in range(self.num_bands):
            # Filtering on, then dropping the `band_index` column
    ...
    def _generate_word_shingles(
        ...
        # self.logger.info(">>>>>>>>>>>>>>>>>en(row)>>")
        # self.logger.info(len(row))
        # self.logger.info(row)


    
        #  minhashes                       ┆ contents                        ┆ document_length ┆ int_id_column
        preprocessed=False
        if len(row) >= 4:
            # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>_generate_word_shingles_generate_word_shingles_generate_word_shingles>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            preprocessed = True
            ...
            # if (k_shingles ==None and doc_len==None):

            #     print(">>>>>>>>>>>>>>>>>BBBB>>>>>>>>>>>>>>>>>>>>>>>>>_generate_word_shingles_generate_word_shingles_generate_word_shingles>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            ...
            #     for i in range(0, max(1, word_count - window_size + 1)):
            #         k_shingles.append(delimiter.join(words[i: i + window_size]))
            #     return k_shingles, doc_len, document_id, preprocessed,inverteFileIndex
            # else:
            return k_shingles, doc_len, document_id,preprocessed
        else:
            ...
            # Compute and print elapsed nanoseconds
            elapsed_ns = t_end - t_start
            # print(f"Elapsed time>>>>>>>>>>>>>>>>>>>SHINGLING>>>>>>>>>>>>>>>>>>>>>>>>>>>>>: {elapsed_ns} ns")
            # return k_shingles, doc_len, document_id,preprocessed,inverteFileIndex
            return k_shingles, doc_len, document_id,preprocessed
