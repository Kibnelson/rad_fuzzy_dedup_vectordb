...
class ServiceOrchestrator:
    ...
    def orchestrate(self) -> int:
        ...
        total_execution_ns = end_time - start_time
        self.logger.info(f"Total execution >>>>>>>>>> {service} time (ns): {total_execution_ns}")
        print(f"Total execution >>>>>>>>>>>>> {service} time (ns): {total_execution_ns}")
        return status
