...
def parallel_minhash_fast(
    ...
    *,
    ...
    # workers = int(n_proc) if n_proc is not None else cpu_cnt
    # workers = max(1, min(workers, N))
    workers = n_proc

    # # Build contiguous, evenly-sized ranges
...
def minhashes_to_vectors(
    ...
    # >>>>Minhash
    # be = mh_uint64.astype('>u8').view(np.uint8)
    # vec   = mh_uint64.astype(">u4").view(np.uint8).reshape(1, -1)
    # return vec

    mh_arr = np.asarray(mh_uint64)
    mh_u32 = mh_arr.astype(np.uint32, copy=False)   # (N, K)
    be = mh_u32.astype(">u4").view(np.uint8)        # bytes
    X = be.reshape(mh_u32.shape[0], -1)             # (N, 4*K)
    return X
...
def main():
    ...
    # Search sweep
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[1000])
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[500,1000,2000,3000,4000])
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[700,500,400,300])
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[900,700,500,400])
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[700,600,500,400,300])
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[900,700,500,400, 300, 200, 100])
    ap.add_argument("--ef_list", type=int, nargs="+", default=[300,400,500,700,900])
    
    # ap.add_argument("--ef_list", type=int, nargs="+", default=[10,16,32,40,48,64])
